...
function Student(stdName, stdAge) {
  this.name = stdName;
  this.age = stdAge;
  this.id = students.length === 0 ? 0 : students[students.length - 1].id + 1;
}

Student.prototype.getIndex = function () {
  return students.findIndex((student) => student.id === this.id);
};
...
function renderDOM() {
  ...
  students.forEach((student, i) => {
    ...
    removeBtn.setAttribute(
      "onclick",
      `handleRemoveStudent(${student.getIndex()})`
    );
